<script lang="ts">
    ...
    import DeleteOff from 'svelte-material-icons/DeleteOff.svelte';
    ...
    >
      ...
      <div class="flex items-center justify-end">
        <div class="flex items-center md:mr-8">
          <CircleIconButton title="Reset faces" logo={Restart} on:click={() => dispatch('reset-delete')} />
          <CircleIconButton
            title="Toggle delete / restore all"
            logo={toggleVisibility ? Delete : DeleteOff}
            on:click={() => dispatch('toggle-delete')}
          />
        </div>
